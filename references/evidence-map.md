...
The skill was distilled from a refined Zotero corpus. The original project folder is provenance only and is not required for normal use.
...
The GitHub skill bundles the distilled indexes, not the original PDFs or all 175 full deconstruction files. A remote-only agent can use the distilled evidence, but it cannot honestly redo or audit a paper-level reading without the corresponding PDF or full deconstruction being supplied.
...
Run `python scripts/search_evidence.py <query>` from the skill folder, or read the relevant bundled evidence index directly.
